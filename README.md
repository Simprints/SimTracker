...
SimTracker is a fork of the [DHIS2 Android app](https://github.com/dhis2/dhis2-android-capture-app) that adds an integration with the [Simprints ID app](https://github.com/Simprints/Android-Simprints-ID) to use its biometrics features.

General scheme of SimTracker interactions with other software components and roles:
...
│ │   SimTracker   │ --------------> │ Simprints ID │ <----------------- Beneficiary
...
* [How DHIS2 and Simprints data models combine and interact in SimTracker](#how-dhis2-and-simprints-data-models-combine-and-interact-in-simtracker)
* [DHIS2 backend instance configuration for SimTracker](#dhis2-backend-instance-configuration-for-simtracker)
...
There are 4 biometrics features that are available in SimTracker at the prototype stage, given that the DHIS2 backend instance which the app user is signed into is configured accordingly (see below about that):
...
In the UI/UX of the SimTracker app, there biometric features are available in the following places:
...
SimTracker syncs linkages between DHIS2 program unique identifiers (UIDs) and Simprints Project IDs, and linkages between DHIS2 organizational unit UIDS and Simprints Module IDs, from the backend of the DHIS2 instance that the SimTracker user is signed into.

For each Program, as described for the UI section above, there are a couple more configurable objects:
* Simprints biometric matching minimum score threshold - to let SimTracker determine whether a given biometric result qualifies as a positive (successful) biometric match or not.
* Biometric locking timeout - to let SimTracker determine for how long to consider a TEI profile freely viewable since the last successful biometric match - via Simprints ID-based enrollment (always), verification or identification.
...
## DHIS2 backend instance configuration for SimTracker
...
| `matchConfidenceScoreThreshold`  | Minimal confidence score for SimTracker to consider biometric match a success                             | Integer number |
...
* Absent or non-positive `matchConfidenceScoreThreshold` value would effectively make it 0, so then anything that Simprins ID returns is a match for SimTracker.
...
* Organization Units are hierarchically structured. If an Organization Unit has no Simprints Module ID, then its nearest parent's Simprints Module ID will be used by SimTracker, if available. If the top level Organization Unit has a Simprints Module ID set, that's effectively a default Module ID.
...
When a TEI (beneficiary) is biometrically enrolled via the Simprints ID app, the latter returns a unique ID (GUID) for the beneficiary. Simprints ID will need this GUID later when called from SimTracker for verification of that TEI or for identification. So TEI needs to have a data field for storing the Simprints GUID. In the DHIS instance admin's web UI, such a new field for TEIs within a given healthcare Program can be added, as a tracked entity attribute. SimTracker requires its short name to be `simprintsGuid`.
...
Considering that SimTracker is a fork of the original DHIS2 upstream, it is effectively a diff from the original DHIS2. The upstream may change considerably over time. Therefore, to keep the complexity of SimTracker development in check, it may be useful to:
...
A [sync-from-upstream](https://github.com/Simprints/SimTracker/blob/main/.github/workflows/sync-from-upstream.yml) GitHub Action periodically creates a pull request to merge the changes from the upstream into this fork.
